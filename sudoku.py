...
class sudoku():
    ...
    class board():
        ...
        def _set_value(self, position, value): #position(0-80)
            ...
            col_row_grid = self._get_col_row_grid(position)
            for pos in set(col_row_grid[0] + col_row_grid[1] + col_row_grid[2]):
                if pos != position:
                    if self.final[pos] == -1:
                        state.possibilities[pos][value - 1] = 0

            return self._set_value_loop(state)

        def _set_value_loop(self, state):
            """ Recrusive loop to turn all singletons into final values (and then reset)"""
            singleton_positions = state._get_singletons()
            while len(singleton_positions) > 0:
                new_pos = singleton_positions[0]
                state = state._set_value(new_pos, state._get_possible_values(new_pos)[0])
                singleton_positions = state._get_singletons()

            return state

    def _pick_next_position(self, partial_state):
    ...
    def _order_values(self, partial_state, position):
        """ Returns the possible values to try for a given position and state """
        values = partial_state._get_possible_values(position)
        return values
    
    def _depth_first_search(self, partial_state):
        """ Perform depth first search, starting with least possibles / lowest values (to force answer)"""

        position = self._pick_next_position(partial_state)
        ...
        values = self._order_values(partial_state, position)

        for value in np.sort(values)[::-1]:
            new_state = partial_state._set_value(position, value)
            if new_state._is_goal():
                return new_state
            if not new_state._is_invalid():
                deep_state = self._depth_first_search(new_state)
                if deep_state is not None and deep_state._is_goal():
                    return deep_state
        return None
    ...
    def sudoku_solver(self, sudoku):
        ...
        partial_state = self.board(sudoku)

        singleton_positions = partial_state._get_singletons()
        while len(singleton_positions) > 0:
            new_pos = singleton_positions[0]
            partial_state = partial_state._set_value(new_pos, partial_state._get_possible_values(new_pos)[0])
            singleton_positions = partial_state._get_singletons()

        if partial_state._is_goal():
            return partial_state._is_valid_answer()

        answer = self._depth_first_search(partial_state)
        return np.full((9,9),-1) if answer == None else answer._is_valid_answer()
